...
# model settings
model = dict(
    type="VoxelNet_Fusion",
    pretrained=None,
    paint_img_features=True,
    image_config=dict(
        config_file='/opt/sdatmp/lq/project/segproject/MVP/c2_config/nuImages_CenterNet2_DLA_640_8x.yaml',
        MODEL=dict(WEIGHT='/opt/sdatmp/lq/project/segproject/MVP/centernet2_checkpoint.pth'),
    ),
    reader=dict(
    ...
    backbone=dict(
        type="SpMiddleResNetFHD", num_input_features=69, ds_factor=8, init_channel=16),     ##todo original(16) no init_channel=32
    neck=dict(
